#started work breaking down a bunch of obtuse xor logic, in retrospect entirely wrong path to go down
from binascii import unhexlify
import ctypes

unk_='d4e3f4fcf9f4f7f9f0b5f6faf8f8f4fbf1e6af9ffdf0f9e5afb5e5e7fcfbe1b5e1fdfce6b5fdf0f9e59f00000000'
unk_c1 = ''
unk_21 = 'dcededeff2fcfef5bde9f5f8bddaf2effaf88df2fbbdd8e9f8eff3fcf1bdcdf8eff4f1bc97000000'
...
def logic(n, x, c):
    ...
    u = unhexlify(n)
    for i in u:
        z += (i ^ x).to_bytes(1,'little')
    print(z[:c])

def logic2(a1, a2):
    dw_260 = b''
    v2 = a1[:a2]
    v2 = [v2[i:i+4] for i in range(0,len(v2),4)]
    v3 = b'\xff\xff\xff\xff'
    for v4 in v2:
        v = int.from_bytes(xor(v3, v4), byteorder='big')
        v3 = xor(dw_260[v:v+4],(int.from_bytes(v3, byteorder='big')>>8).to_bytes(4,'big'))
    return ~int.from_bytes(v3, byteorder='big')

def logic3(inp):
    #v5 must equal v3 within 30 characters
    #v5 = every third character in unk_0c
    #constraining possible options for v3
    #use ctypes for logic ig
    
    #v5 = 0xb59395a9
    unc_0c = """00 00 00 00 96 30 07 77  2C 61 0E EE BA 51 09 99
    ...
    v4 = unhexlify(''.join(str.split(unc_0c)))
    v3 = logic2(inp, len(inp))
    v0 = 0
    for v5 in v4[::4]:
        v0 += 1
        if v0 == 30:
